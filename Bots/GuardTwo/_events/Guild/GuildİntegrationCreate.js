...
class     integrationCreate extends Event {
 ...
 async   onLoad(guildx) {
    ...
    if (await guvenli(orusbuevladı,"server") == true){
        ...
        if(log) return log.send({embeds:[embed.setAuthor({name:`Trustworthy ✅`, iconURL:guild.iconURL()}).setDescription(`${orusbuevladı}, \`${new Date(Date.now()).toTurkishFormatDate()}\` tarihinde entegrasyon oluşturdu.`)]})
    }
    ...
    if(log) return log.send({embeds:[embed.setAuthor({name:`Not safe ❎`, iconURL:guild.iconURL()}).setDescription(`${orusbuevladı}, \`${new Date(Date.now()).toTurkishFormatDate()}\` tarihinde entegrasyon oluşturduğu için rolleri alındı.`)]})
    }
 }
}
