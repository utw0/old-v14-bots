...
class channelUpdate extends Event {
 ...
 async  onLoad(oldChannel, newChannel) {
    ...
    await guardPenaltyDB.findOneAndUpdate({guildID:guild.id,OrusbuEvladı:orusbuevladı.id},{$push:{işlemler:{Güvenilir:true,işlem:`Kanal Güncelleme (İsmi)`,Tarih:Date.now()}}},{upsert:true})
      if(log) return log.send({embeds:[embed.setAuthor({name:`Trustworthy ✅`, iconURL:guild.iconURL()}).setDescription(`${orusbuevladı}, \`${new Date(Date.now()).toTurkishFormatDate()}\` tarihinde **${oldChannel.name}** isimli kanalı **${newChannel.name}** olarak değiştirdi.`)]})
    } 
    if(oldChannel.parentId != newChannel.parentId){
      await guardPenaltyDB.findOneAndUpdate({guildID:guild.id,OrusbuEvladı:orusbuevladı.id},{$push:{işlemler:{Güvenilir:true,işlem:`Kanal Güncelleme (Kategori Değiştirildi)`,Tarih:Date.now()}}},{upsert:true})
        if(log) return log.send({embeds:[embed.setAuthor({name:`Trustworthy ✅`, iconURL:guild.iconURL()}).setDescription(`${orusbuevladı}, \`${new Date(Date.now()).toTurkishFormatDate()}\` tarihinde **${oldChannel.name}** kanalını \`${newChannel.guild.channels.cache.get(newChannel.parentId).name}\` kategorisine taşıdı.`)]})
      }
    if(newChannel.type == 2 && (oldChannel.userLimit != newChannel.userLimit)){
      ...
      if(log) return log.send({embeds:[embed.setAuthor({name:`Trustworthy ✅`, iconURL:guild.iconURL()}).setDescription(`${orusbuevladı}, \`${new Date(Date.now()).toTurkishFormatDate()}\` tarihinde **${oldChannel.name}** kanalının limiti \`${newChannel.userLimit}\` olarak değiştirildi.`)]})
    } 
    if((newChannel.type === 0 || (newChannel.type === 5)) && (oldChannel.topic != newChannel.topic)){
      ...
      if(log) return log.send({embeds:[embed.setAuthor({name:`Trustworthy ✅`, iconURL:guild.iconURL()}).setDescription(`${orusbuevladı}, \`${new Date(Date.now()).toTurkishFormatDate()}\` tarihinde **${oldChannel.name}** kanal açıklaması \`${newChannel.topic}\` olarak değiştirildi.`)]})
    }
    ...
    if(log) return log.send({embeds:[embed.setAuthor({name:`Trustworthy ✅`, iconURL:guild.iconURL()}).setDescription(`${orusbuevladı}, \`${new Date(Date.now()).toTurkishFormatDate()}\` tarihinde **${oldChannel.name}** isimi kanal güncellendi!`)]})
