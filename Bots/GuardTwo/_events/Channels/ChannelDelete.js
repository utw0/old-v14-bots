...
class channelDelete extends Event {
 ...
 async  onLoad(channel) {
    ...
    if ((channel.type === 0) || (channel.type === 5)) {
      ...
      if(log) log.send({embeds:[embed.setAuthor({name:`Not safe ❎`, iconURL:guild.iconURL()}).setDescription(`${orusbuevladı}, \`${new Date(Date.now()).toTurkishFormatDate()}\` tarihinde **${channel.name}** isimli kanalı sildiği için kendisini yasakladım ve kanalı geri açıp izinlerini ayarladım!`)]})
    ...
    if (channel.type === 2) {
      ...
      if(log) log.send({embeds:[embed.setAuthor({name:`Not safe ❎`, iconURL:guild.iconURL()}).setDescription(`${orusbuevladı}, \`${new Date(Date.now()).toTurkishFormatDate()}\` tarihinde **${channel.name}** isimli Ses kanalı sildiği için kendisini yasakladım ve kanalı geri açıp izinlerini ayarladım!`)]})
    ...
    if (channel.type === 4) {
      ...
      if(log) log.send({embeds:[embed.setAuthor({name:`Not safe ❎`, iconURL:guild.iconURL()}).setDescription(`${orusbuevladı}, \`${new Date(Date.now()).toTurkishFormatDate()}\` tarihinde **${channel.name}** isimli Kategoriyi sildiği için kendisini yasakladım ve kanalı geri açıp izinlerini ve kanallarını ayarladım!`)]})
