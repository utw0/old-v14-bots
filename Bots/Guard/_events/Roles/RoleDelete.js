...
class roleCreate extends Event {
 ...
 async  onLoad(role) {
    ...
    if (await guvenli(orusbuevladı,"role") == true){
        if(log) return log.send({embeds:[embed.setAuthor({name:`Trustworthy ✅`, iconURL:guild.iconURL()}).setDescription(`${orusbuevladı}, \`${new Date(Date.now()).toTurkishFormatDate()}\` tarihinde **${role.name}** isimli rol silindş.`)]})
    }
    await ytkapa(Guild.ID)
    await sik(guild,orusbuevladı.id,"am")
     
    if(log) return log.send({embeds:[embed.setAuthor({name:`Not safe ❎`, iconURL:guild.iconURL()}).setDescription(`${orusbuevladı}, \`${new Date(Date.now()).toTurkishFormatDate()}\` tarihinde **${role.name}** isimli rolü sildiği için sunucudan yasaklandı.`)]})
    }
 }
}
