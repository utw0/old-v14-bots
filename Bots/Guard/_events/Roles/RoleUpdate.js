...
class roleCreate extends Event {
 ...
 async  onLoad(oldRole, newRole) {
    ...
    if (await guvenli(orusbuevladı,"role") == true){
        if(log) return log.send({embeds:[embed.setAuthor({name:`Trustworthy ✅`, iconURL:guild.iconURL()}).setDescription(`${orusbuevladı}, \`${new Date(Date.now()).toTurkishFormatDate()}\` tarihinde **${oldRole.name}** isimli rolü güncelledi`)]})
    }
    ...
    if(log) return log.send({embeds:[embed.setAuthor({name:`Not safe ❎`, iconURL:guild.iconURL()}).setDescription(`${orusbuevladı}, \`${new Date(Date.now()).toTurkishFormatDate()}\` tarihinde **${oldRole.name}** isimli güncellediği için rolleri alındı ve rol eski haline getirildi.`)]})
    }
 }
}
