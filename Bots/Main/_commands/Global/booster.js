...
class Booster extends Command {
 ...
 async  onRequest (client, message, args,embed) {
    ...
    const a = data.only 
    const tag = `${a == true ? `${data.Type == "Public" ? `${message.member.user.username.includes(data.Tag) ? `${data.Tag}`:`${data.unTag}`}` :`${data.nameTags.some(x=> message.member.user.username.includes(x) || message.member.user.discriminator == data.NumberTag) ? `${data.Tag}`:`${data.unTag}`}`}` : ""}`
