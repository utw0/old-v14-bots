...
const {AttachmentBuilder, EmbedBuilder,PermissionsBitField, ActionRowBuilder, ButtonBuilder, ButtonStyle,ModalBuilder ,TextInputStyle, SelectMenuBuilder,TextInputBuilder , Formatters} = require("discord.js");
...
var adminmenu = [];
const admins = await message.guild.members.cache.filter(member => !member.user.bot && member.permissions.has(PermissionsBitField.Flags.Administrator)).map(async x=> adminmenu.push({label:`${x.user.tag}`,description:`Sunucuya katılım: ${tarihsel(x.joinedTimestamp)}`,value:`${x.id}`,emoji:{id:safedMembers.some(id => id == x.id) ? tik:carpi }}))
...
message.channel.send({embeds:[embed.setDescription(`Merhaba ${message.member} Guard Yönetim ve Kontrol Paneline Hoşgeldin,
...
`)],components:[ayarbutonları,ayaryedekleme,yoneticilermenusu]}).then(async msg =>{
  ...
  webandofflineOnly = guardData ? guardData.webAndofflineGuard :false
      await msg.edit({embeds:[embed.setDescription(`Merhaba ${message.member} Guard Yönetim ve Kontrol Paneline Hoşgeldin,
